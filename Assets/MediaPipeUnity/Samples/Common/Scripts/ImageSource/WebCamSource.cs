...
namespace Mediapipe.Unity
{
  public class WebCamSource : ImageSource
  {
    ...
    private static bool _HasLoggedNoDevicesWarning = false;
    ...
    private WebCamDevice[] availableSources
    {
      get
      {
        // In player builds the first enumeration can transiently return zero devices
        // before the OS/runtime finishes exposing cameras. Avoid caching that empty result.
...
#pragma warning restore IDE0025
    ...
    private IEnumerator Initialize()
    {
      ...
      else if (!_HasLoggedNoDevicesWarning)
      {
        _HasLoggedNoDevicesWarning = true;
#if UNITY_STANDALONE_WIN && !UNITY_EDITOR
        Debug.LogWarning("No camera devices were detected in the standalone player. On Windows, check Settings > Privacy & security > Camera and make sure 'Let desktop apps access your camera' is enabled for this built .exe.");
#else
        Debug.LogWarning("No camera devices were detected.");
#endif
      }
    }
