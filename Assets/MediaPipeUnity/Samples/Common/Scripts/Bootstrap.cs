...
namespace Mediapipe.Unity.Sample
{
  public class Bootstrap : MonoBehaviour
  {
    ...
    private string _webCamDebugText = "Enumerating cameras...";
    private float _nextWebCamDebugRefreshTime;
    private GUIStyle _webCamDebugStyle;
#endif
    ...
    private void Update()
    {
      if (Time.unscaledTime >= _nextWebCamDebugRefreshTime)
      {
        _nextWebCamDebugRefreshTime = Time.unscaledTime + 1f;
        RefreshWebCamDebugText();
      }
    }

    private void OnGUI()
    {
      if (_webCamDebugStyle == null)
      {
        _webCamDebugStyle = new GUIStyle(GUI.skin.box)
        {
      ...
      var area = new UnityEngine.Rect(
        16,
        16,
        Mathf.Min(UnityEngine.Screen.width - 32, 720),
        Mathf.Min(UnityEngine.Screen.height * 0.35f, 260));
      GUI.Box(area, _webCamDebugText, _webCamDebugStyle);
    }

    private void RefreshWebCamDebugText()
    {
      var devices = WebCamTexture.devices;
      var builder = new StringBuilder();
      builder.AppendLine("Standalone Webcam Debug");
      builder.AppendLine($"Count: {devices.Length}");

      for (var i = 0; i < devices.Length; i++)
      {
        var device = devices[i];
        builder.AppendLine($"{i}: {device.name}");
      }

      if (devices.Length == 0)
      {
        builder.AppendLine("No webcam devices reported by Unity.");
      }

      builder.AppendLine();
      builder.AppendLine(ImageSourceConfig.DebugState);

      _webCamDebugText = builder.ToString();
    }
#endif
  }
}
